...
void TCSOLVE(){
    ...
    vector<int> pge = previousgreaterelement(v);
    ...
    */
    ...
    for(int i = 0;i<n;i++){
        ...
        minimums = (minimums + (cnse*cpse*1ll*v[i])%mod)%mod;
        maximums = (maximums + (cnge*cpge*1ll*v[i])%mod)%mod;
    }

    cout << maximums - minimums << endl;


    // instead of figuring out every subarray and then fetching the maximum and minimum in that and substracting them and on a whole summing up net subs in all subarrays, we just do reverse engineering, in how many subarrays does x contribute to the ans i.e, as maximum and as minimum separately
